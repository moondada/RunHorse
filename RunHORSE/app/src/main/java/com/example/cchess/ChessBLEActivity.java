...
public class ChessBLEActivity extends Activity {
	...
	private ChessView chess;//动作在视图上的体现
		...
		chess.setLisenner(new onStepLisenner() {
			...
			public void onStep(int oldx, int oldy, int cx, int cy, Chess_Item_Base oldItem, List<Chess_Item_Base> items) {
				...
				if (getConStatus()) {
					...
					status.setCx(8 - cx);
					status.setCy(9 - cy);
					status.setOldx(8 - oldx);
					status.setOldy(9 - oldy);
					if(oldItem!=null){
						chess.setOldItem();
                        Chess_Item_Base newItem = getNewItem(oldItem,items);
                        status.setItem(newItem);
						if(status.getNx()!=-1)
							chess.setIsPlaying(true);
							chess.eatItem(status.getNx(),status.getNy(),status.getItype());
                    }
					...
					if (status.isIschange()) {
						new Thread(new Runnable() {
							...
							public void run() {
								...
								if(status.getNx()!=-1)
								{
									chess.setIsPlaying(false);
									chess.eatItem(8-status.getNx(),9-status.getNy(),status.getItype());
									chess.setOldItem();
								}
						...
						chess.setItems(status.getItems2(),status.getState2());
						chess.change();
	...
	public void onClick(View view) {
		...
		case R.id.btn_change: 
			...
			if (getConStatus()) {
				...
				status.setItems();
				chess.setItems(status.getItems(),status.getState());
				status.setMessage("");
	...
	public Chess_Item_Base getNewItem(Chess_Item_Base oldItem, List<Chess_Item_Base> items){
		Chess_Item_Base item = oldItem;
		if(item.getType() == 2){
			item.setCentx(4);
			item.setType(2);
			if(item.isRed())
